var joinValidate = {
		...
		resultCode : {
			...
			empty_id : {
				code : 1,
				//desc : 'ID를 입력해 주세요.'
				desc : '회원 아이디를 입력해 주세요.'
			},
			space_length_id : {
				code : 2,
				desc : '아이디는 공백없이 6자 이상~50자 이하여야 합니다.'
			},
			invalid_id : {
				code : 3,
				//desc : '6~50자의 영문 소문자, 숫자, 특수기호 일부만 사용할 수 있습니다.'
				desc : '아이디는 영문 소문자, 숫자, 특수기호 일부만 사용할 수 있습니다.'
			},
			email_id : {
				code : 4,
				desc : '이메일 아이디를 사용할 수 없습니다.'
			},
			special_char_id : {
				code : 5,
		...
		checkEmail: function(em) {
			...
			var lastIndex = em.lastIndexOf("@");
			if (-1 == lastIndex){
				return this.resultCode.invalid_email;
			}
			var emId = em.substring(0, lastIndex);
			if (emId == "") {
				return this.resultCode.invalid_email;
			}

			var emLength = em.length;
			var blankRemovedLength = (em.replace(/\s/gi, "").replace(/\r/gi, "").replace(/\n/gi, "")).length;
			var excludeBlank = emLength == blankRemovedLength;
			var result = excludeBlank;
			if( result )
	    ...
	    checkStringId : function(memId) {
			...
			} else if( (memId.indexOf("@") > 0) && (memId.indexOf(".") > 0) ) {
				return this.resultCode.email_id;
    		...
    		/*
			if(jQuery.trim(memId) == '' ||  jQuery.trim(memId).length <= 0){
				return this.resultCode.empty_id;
			} else {
				var idCheck	= /^[a-z0-9]+[a-z0-9-._]{5,49}$/;
				var idCheck2	= /^[0-9-._]{5,49}$/;
				var isNumeric = this.isNumerics(memId);
				var special_pattern = /[`~!@#$%^&*|()=+<>,\\\'\";:\/?]/gi;
				if (memId.indexOf(' ') > -1) {
					return this.resultCode.space_length_id;
				}
				if( (memId.indexOf("@") > 0) && (memId.indexOf(".") > 0) ) {
					return this.resultCode.email_id;
				}
				if (!idCheck.test(memId.toLowerCase())
						|| idCheck2.test(memId.toLowerCase())
						|| isNumeric) {
					if( memId.charAt(0) == '-' || memId.charAt(0) == '_' || memId.charAt(0) == '.' ) {
						return this.resultCode.first_special_char_id;
					}
					if( special_pattern.test(memId.toLowerCase()) ) {
						return this.resultCode.special_char_id;
					}
					return this.resultCode.invalid_id;
				}
				if( memId.length < 6 || memId.length > 50 ) {
					return this.resultCode.space_length_id;
				}
				return this.resultCode.success_id;
			}
...
var safety = {
		...
		scriptMsg : {
			...
			password_length2 : {
				desc : '비밀번호는 20자리 이하로 입력해 주세요.'
			},
			password_prohibit1 : {
				desc : '영문+숫자 조합으로 설정하셔야 합니다.'
			},
			password_prohibit2 : {
				desc : '아이디로 비밀번호 사용은 불가합니다.'
			},
			password_prohibit3 : {
				desc : '111처럼 동일한 숫자, 문자는 사용하실 수 없습니다.'
			},
			password_prohibit4 : {
				desc : '123처럼 연속된 숫자, 문자는 사용하실 수 없습니다.'
			},
			password_prohibit5 : {
				desc : '내 정보로 비밀번호 사용은 사용하실 수 없습니다.'
			},
	     ...
	     * @returns
	     */
		checkPwdCompareTarget: function (targetStr, compStr, length) {
	    	if (targetStr == null || targetStr == '') {
	    		return "SUCC";
			}
			for (var i = 0 ; i < compStr.length - length + 1 ; i++) {
